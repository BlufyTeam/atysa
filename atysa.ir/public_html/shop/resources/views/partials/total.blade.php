...
<div class="osahan-cart-item rounded rounded shadow-sm overflow-hidden bg-white sticky_sidebar">
    ...
    @if(Auth::check())
        @if(isset(Auth::user()->addresses) && Auth::user()->addresses != null)
            <div class="d-flex border-bottom osahan-cart-item-profile bg-white p-3">
                <div class="d-flex flex-column">
                    ...
                    <div class="d-flex align-items-center justify-content-between px-3 py-2 border-bottom">
                        <div class="media align-items-center">
                            <div class="mr-2 text-danger">
                                <input type="radio" {{$key == 0 ? 'checked' : ''}} class="form-control" name="address" value="{{$address['address']}} - {{$address['area']}}"/>
                            </div>
                            <div class="media-body">
                                <p class="m-0"> {{$address['address']}} - {{$address['area']}}</p>
                            </div>
                        </div>
                    </div>
                    @endforeach
            ...
            <a data-toggle="modal" data-target="#exampleModal"  class="btn btn-primary btn-block btn-sm" >افزودن آدرس</a>
        @else
        <a data-toggle="modal" data-target="#exampleModal"  class="btn btn-primary btn-block btn-sm" >ابتدا آدرس درج نمایید<i class="feather-arrow-left"></i></a>
        @endif
    @endif
    ...
    <div class="p-3">
        ...
        @else
            <button @if(count($userBasket) == 0) disabled @endif type="submit" class="btn btn-success btn-block btn-lg" >پرداخت<i class="feather-arrow-right"></i></button>
        @endif
    </div>
</div>
</form>