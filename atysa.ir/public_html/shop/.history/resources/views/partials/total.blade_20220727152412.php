    ...
    @if(Auth::check())
        @if(isset(Auth::user()->addresses) && Auth::user()->addresses != null)
            <div class="d-flex border-bottom osahan-cart-item-profile bg-white p-3">
                    ...
                    <div class="d-flex align-items-center justify-content-between px-3 py-2 border-bottom">
                        <div class="media align-items-center">
                            <div class="mr-2 text-danger">
                                <input type="radio" {{$key == 0 ? 'checked' : ''}} class="form-control" name="address" value="{{$address['address']}}"/>
                            </div>
                            <div class="media-body">
                ...
                <a data-toggle="modal" data-target="#exampleModal"  class="btn btn-primary btn-block btn-sm" >افزودن آدرس<i class="feather-arrow-right"></i></a>
            </div>
        @else
        <a data-toggle="modal" data-target="#exampleModal"  class="btn btn-primary btn-block btn-sm" >ابتدا آدرس درج نمایید<i class="feather-arrow-right"></i></a>
        @endif
    @endif
